...
# --- 1. CONFIGURATION DU PROXY (INDISPENSABLE SUR LE WEB) ---
# On utilise st.get_option pour s'adapter au port réel du serveur Cloud
if 'STREAMLIT_RUNTIME__IS_RELEASE' in os.environ:
    os.environ['LOCALTILESERVER_CLIENT_PREFIX'] = f"proxy/{st.get_option('server.port')}"
...
with col_map:
    ...
    # --- RENDU RASTER (CORRECTION FINALE) ---
    # On récupère les chemins absolus pour le serveur Linux de Streamlit
    hillshade_path = os.path.join(os.getcwd(), "Hillshade.tif")
...
with col_stats:
    ...
    fig_bar.update_layout(xaxis={'type': 'category'}, height=300)
